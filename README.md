...
The R script `GEN_IMAGENES.R` reads an Excel file with the stock data, generates listings by store, and saves the results as text files and graphs in PNG format.
...
The Python script HERRRAMIENTA_AUDIO.py reads the text files generated by the R script and converts them into audio files using gTTS.
...
![Audio Example](tienda_3_example.mp3)
